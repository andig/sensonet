...
func NewSensonetFromConfig(other map[string]interface{}) (api.Charger, error) {
	...
	return NewSensonet(cc.embed, cc.User, cc.Password, cc.PvUseStrategy, cc.HeatingZone, cc.Phases, cc.HeatingTemperatureOffset, cc.StandbyPower)
}

// NewSensonet creates Sensonet charger
func NewSensonet(embed embed, user, password, pvUseStrategy string, heatingZone, phases int32, heatingTemperatureOffset, standbypower float64) (*Sensonet, error) {
	conn, err := sensonet.NewConnection(user, password, pvUseStrategy, heatingZone, phases, heatingTemperatureOffset)
	if err != nil {
		return nil, err
	}
